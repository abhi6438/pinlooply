...
// ── Editable task row ─────────────────────────────────────────
function TaskRow({ task, index, onUpdate, onDelete }) {
    ...
    return (
      <div className="border border-indigo-200 rounded-lg p-3 bg-indigo-50/30 space-y-2">
        ...
        <div className="flex gap-2">
          <button onClick={save} className="flex items-center gap-1 text-xs bg-indigo-600 text-white px-2.5 py-1 rounded hover:bg-indigo-700">
            <Check className="w-3 h-3" /> Save
          </button>
          <button onClick={() => setEditing(false)} className="flex items-center gap-1 text-xs text-gray-500 px-2 py-1 rounded hover:bg-gray-100">
...
export default function AIConfirm() {
  ...
  async function handleSave() {
    setSaving(true)
    try {
      const response = await discussionsApi.save(rawText, projectId, source, {
        ...aiResult,
        topics,
        tasks,
        conflicts,
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6">
      ...
      <div className="space-y-4">
        ...
        <Section icon={Tag} title="Topics Found" count={topics.length} color="text-indigo-500">
          {topics.length > 0 ? (
            topics.map((t, i) => (
              <TopicRow key={i} topic={t} index={i} onDelete={deleteTopic} />
            ))
          ) : (
        ...
        <Section icon={ListChecks} title="Tasks Extracted" count={tasks.length} color="text-blue-500">
          {tasks.length > 0 ? (
            tasks.map((t, i) => (
              <TaskRow key={i} task={t} index={i} onUpdate={updateTask} onDelete={deleteTask} />
            ))
          ) : (
                ...
                <div key={i} className="bg-orange-50 border border-orange-200 rounded-lg p-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div className="bg-white rounded p-2">
                      ...
                      <p className="text-gray-700">{c.old_value}</p>
                    </div>
                    <div className="bg-white rounded p-2">
                      <p className="text-gray-500 mb-1">Now</p>
                      <p className="text-gray-700">{c.new_value}</p>
                    </div>
                  </div>
                </div>
              ))}
      ...
      <div className="flex gap-3 mt-6 sticky bottom-4">
        <button
          ...
          className="flex items-center gap-2 border border-gray-300 text-gray-700 px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-gray-50 transition-colors bg-white shadow-sm"
        >
        ...
        <button
          onClick={handleSave}
          disabled={saving}
          className="flex-1 flex items-center justify-center gap-2 bg-indigo-600 text-white px-5 py-2.5 rounded-lg text-sm font-medium hover:bg-indigo-700 transition-colors disabled:opacity-60 shadow-sm"
        >
