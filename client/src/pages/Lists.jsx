...
function QuickAddRow({ projects, activeType, onAdd, onCancel }) {
  ...
  async function submit(e) {
    ...
    setSaving(true)
    try {
      await tasksApi.create({ title: title.trim(), type: activeType, priority, project_id: projectId, due_date: dueDate || null })
...
// ── Task Row ──────────────────────────────────────────────────
function TaskRow({ task, selected, onSelect, onToggleDone, onDelete, onUpdate, onNavigateTopic }) {
      ...
      <tr className="bg-indigo-50/40 border-l-2 border-indigo-400">
        ...
        <td className="pr-4 py-2.5">
          <div className="flex gap-1">
            ...
            <button onClick={cancelEdit} className="text-xs text-gray-500 px-2 py-1 rounded hover:bg-gray-100">
              Cancel
            </button>
          </div>
        </td>
          ...
          {priorityOpen && (
            <div className="absolute right-0 top-9 bg-white rounded-lg shadow-xl border border-gray-200 py-1 z-30 min-w-[110px]">
              {[
                ...
                <button
                  key={p.value}
                  onClick={() => { onPriority(p.value); setPriorityOpen(false) }}
...
export default function Lists() {
  ...
  async function onTaskAdded() {
    setShowAddForm(false)
    await loadTasks()  // reload from server — guaranteed to show new task
  }
