  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 mt-2">Let's get you set up in under 2 minutes.</p>
      </div>
      <div>
  ...
  async function handleStep4() {
    ...
    try {
      const { data: grp, error } = await supabase
        .from('groups')
        .insert({ name: group.name, owner_id: user.id })
        .select()
        .single()
      if (error) throw error
      await supabase.from('group_members').insert({ group_id: grp.id, user_id: user.id, role: 'owner' })
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
          ...
          {step === 1 && <StepName value={name} onChange={setName} onNext={handleStep1} />}
